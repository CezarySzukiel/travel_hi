import {
    Card, CardContent, CardActions, Button, Typography, Box, CircularProgress, Alert, Stack, Chip,
} from "@mui/material";

import type {Sample, DetectResult} from "../types/Transport"
...
export default function DetectTransportTile({detectSeconds = 10, onComplete}: Props) {
    ...
    const stopAndEstimate = () => {
        ...
        const res = {mode, speedKmh, samples};
        setResult(res);
        setDetecting(false);
        onComplete?.(res);
    };
    ...
    return (
        <Card sx={{height: "100%"}}>
            ...
            <CardActions>
                {!detecting ? (
                    <Button variant="contained" onClick={startDetect}>Start</Button>
                ) : (
